...
def particles_BCs(cfg: dict[str, T.Any], xg: dict[str, T.Any]):
    ...
    if "E0precip_function" in cfg:
        if (cfg["nml"].parent / (cfg["E0precip_function"] + ".py")).is_file():
            func_path = cfg["nml"].parent
        Efunc = str2func(cfg["E0precip_function"], func_path)

    Qtmp = Qfunc(pg, cfg["Qprecip"], cfg["Qprecip_background"])
    E0tmp = Efunc(pg, cfg["E0precip"], cfg["E0precip_background"])
    ...
    assert np.isfinite(pg["E0"]).all(), "E energy must be finite"
    assert (pg["E0"] >= 0).all(), "E energy must be non-negative"
